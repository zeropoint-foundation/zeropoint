...
export async function verifyRequest(request, env) {
  ...
  // Cookie-based session auth (browser surfaces). The `zp_session`
  // HttpOnly cookie is set by /api/auth/session and the WebAuthn
  // verify endpoint, scoped to .FOUNDATION_DOMAIN so it travels to
  // the apex and every subdomain. Browser surfaces (mail, signin,
  // the chain viz) fetch with `credentials: 'include'` and don't
  // carry an Authorization header — this branch is what makes that
  // work. Returning directors get persistent auth across tab close.
  if (!authHeader) {
    const session = await import("./session.js");
    const cookieHeader = request.headers.get("Cookie");
    if (cookieHeader) {
      const token = parseCookie(cookieHeader, session.SESSION_COOKIE_NAME);
      if (token) return session.verifySession(env, token);
    }
    return { ok: false, error: "Missing Authorization header", status: 401 };
  }

  // Session token auth (Bearer) — for browser workspace clients
  ...
  if (!authHeader.startsWith("ZP ")) {
    return { ok: false, error: "Invalid auth scheme (expected ZP or Bearer)", status: 401 };
  }
...
/**
 * Parse a Cookie header for a specific cookie value.
 * Returns the raw value or null. Standard cookie parsing — splits on
 * ";", trims each pair, matches by name.
 */

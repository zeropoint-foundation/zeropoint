...
const ALLOWED_CRATES: &[&str] = &["crates/zp-artifacts/src", "crates/zp-artifacts/tests"];
...
fn no_raw_artifact_construction_outside_zp_artifacts() {
    let root = Path::new(WORKSPACE_ROOT).parent().expect("workspace root");
...
fn scan_dir(dir: &Path, violations: &mut Vec<String>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            if matches!(
                name,
                "target" | ".git" | "node_modules" | ".understand-anything"
            ) {
                continue;
            }
    ...
    for pattern in FORBIDDEN_PATTERNS {
        if content.contains(pattern) {
            ...
            if content.contains("zp_artifacts") || content.contains("zp-artifacts") {
                violations.push(format!("  {} — contains '{}'", path_str, pattern));

...
fn main() {
    ...
    assert!(allowed_result.is_allowed(), "Normal actor should be allowed");
    ...
    println!("\nLow-trust actor (Tier0, min is Tier1): {:?}", low_tier_result.decision);
    assert!(low_tier_result.is_blocked(), "Low-trust actor must be blocked");
    ...
    for i in 0..5 {
        ...
        let status = if result.is_allowed() { "Allow" } else { "Block" };

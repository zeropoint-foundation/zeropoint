...
- chart_generate — produces a self-contained SVG chart from structured data. Params: type (required, one of "bar", "line", "pie"), title (optional string), labels (required array of strings — x-axis for bar/line, slice labels for pie), series (required array of `{"name":"...","values":[...]}` objects — one series for pie, multiple for bar/line where each series's `values` length must equal `labels` length). Returns `{"svg":"<svg>...</svg>","content_hash":"<blake3-hex>","type":"bar"}`. Output is deterministic (identical inputs → byte-identical bytes) and the content hash is chain-anchored via a `regent:tool:artifact:chart_generate` receipt. Use when the operator asks you to visualize data, produce a chart or graph, or when you're composing a report and want an inline chart. Feed the returned `svg` into `report_assemble` as a fragment's `chart_svg` field.
- report_assemble — composes a dark-themed self-contained HTML report from ordered fragments. Params: fragments (required array of `{"heading":"...","body_html":"...","chart_svg":"..."}` objects; only `body_html` is required per fragment). `body_html` may contain paragraphs, lists, and other structural HTML but `<script>` and `<style>` blocks are stripped defensively. `chart_svg` is typically the `svg` returned by a prior `chart_generate` call. Returns `{"html":"<!DOCTYPE...>","content_hash":"<blake3-hex>","fragment_count":N}`. Output is deterministic and the content hash is chain-anchored via a `regent:tool:artifact:report_assemble` receipt. Use when the operator asks for a report, writeup, or synthesis. Compose with `save_to_artifacts` to persist the resulting HTML to the operator's artifact library.
- save_to_artifacts — persists bytes to the operator's artifact library at `~/ZeroPoint/artifacts/<blake3-hash>.<ext>` and emits an `artifact:library:candidate` receipt. Params: name (required bare filename like "competitive-analysis.html" — used for the extension and receipt narration only; the actual filename is content-addressed by hash), and exactly one of content (UTF-8 text like HTML or SVG) or content_base64 (base64-encoded bytes for images/PDFs/any non-UTF-8 content). Returns `{"path":"/abs/path","content_hash":"<hex>","bytes":N,"already_existed":true|false}`. Idempotent: same bytes produce the same filename, so a second call with identical content is a no-op-in-effect (the receipt is still emitted, so the chain records every call). Path-traversal-checked: `name` must be a bare filename with no slashes or `..`. Typical use: pipeline it after `report_assemble` or `chart_generate` — take the returned `html` or `svg` string and pass it as `content`, use a descriptive `name` for the receipt. The operator can then find the file at the returned `path`.
...
"Chart Q3 revenue for Acme, Beta, Cega" → {"intent":"execute","tool":"chart_generate","params":{"type":"bar","title":"Q3 Revenue","labels":["Acme","Beta","Cega"],"series":[{"name":"USD (M)","values":[24.5,18.2,31.7]}]}}
"Plot chain length over the last 5 cycles" → {"intent":"execute","tool":"chart_generate","params":{"type":"line","title":"Chain length by cycle","labels":["c1","c2","c3","c4","c5"],"series":[{"name":"entries","values":[12093,12112,12148,12203,12281]}]}}

...
pub fn run(config: &InitConfig) -> i32 {
    ...
    if config.store_genesis_secret {
        ...
        eprintln!();
        eprintln!("  \x1b[1mSovereignty: {}\x1b[0m", provider.display_name());
        match config.sovereignty_mode {
            ...
            SovereigntyMode::FaceEnroll => {
                eprintln!(
                    "  Your face becomes the key. A compact local template — no images stored,"
                );
...
# Strict schema: unknown sections are rejected at startup.

[server]
# Where the zp gate listens. 127.0.0.1 is the secure default (localhost-only).
bind = "127.0.0.1"
# Primary listen port. zp also binds port+1 as the gRPC pair.
port = 17010

[identity]
# Operator handle — recorded in every receipt the substrate emits.
operator = "{}"
...
[node]
# Node role in the trust topology: "genesis" (this machine owns the keys)
# or "delegate" (forwards to an upstream genesis node).
role = "genesis"
# upstream = "192.168.1.x:17010"  # only needed for delegate nodes
"#,
    ...
    eprintln!();
    eprintln!(
        "  Sovereignty mode: \x1b[1m{}\x1b[0m",
        actual_mode.display_name()
    );

...
use zp_receipt::{ClaimMetadata, ClaimSemantics, ReceiptBuilder, ReceiptType, Status};
...
pub async fn compose_handler(
    ...
    let Some(comp) = chain_narration::build_composition(
        ...
        return (
            ...
            Json(serde_json::json!({"error": "receipts list is empty"})),
        )
            .into_response();
    };
    ...
    let supersedes = match req.supersedes.as_deref() {
        Some(hex) => match ArtifactId::from_hex(hex) {
            ...
            Err(e) => {
                return (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({"error": format!("bad supersedes id: {e}")})),
                )
                    .into_response()
            }
        },
...
pub async fn list_handler(State(state): State<AppState>, Query(q): Query<ListQuery>) -> Response {
    ...
    match state.0.artifact_library.list(&filter).await {
        Ok(ids) => {
            ...
            (
                StatusCode::OK,
                Json(serde_json::json!({"artifact_ids": hex_ids})),
            )
                .into_response()
        }
        Err(e) => library_err(e),
...
pub async fn get_handler(State(state): State<AppState>, Path(id_hex): Path<String>) -> Response {
    let id = match ArtifactId::from_hex(&id_hex) {
        Ok(id) => id,
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "invalid artifact id"})),
            )
                .into_response()
        }
    };
...
pub async fn sign_handler(State(state): State<AppState>, Path(id_hex): Path<String>) -> Response {
    let id = match ArtifactId::from_hex(&id_hex) {
        Ok(id) => id,
        Err(_) => {
            return (
        ...
        .await
    {
        Ok(()) => {
            ...
            (
                StatusCode::OK,
                Json(serde_json::json!({"signed": true, "artifact_id": id_hex})),
            )
                .into_response()
        }
        Err(e) => library_err(e),
...
pub async fn reject_handler(State(state): State<AppState>, Path(id_hex): Path<String>) -> Response {
    let id = match ArtifactId::from_hex(&id_hex) {
        Ok(id) => id,
        Err(_) => {
            return (
    ...
    match state.0.artifact_library.reject(&id).await {
        Ok(()) => (
            StatusCode::OK,
            Json(serde_json::json!({"rejected": true, "artifact_id": id_hex})),
        )
            .into_response(),
        Err(e) => library_err(e),
    }
...
pub async fn canonical_handler(
    ...
    let Some(kind) = parse_kind(&kind_str) else {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"error": "unknown kind — valid: chain_narration"})),
        )
            .into_response();
    };
    let operator_id = &state.0.identity.destination_hash;
    match state
        .0
        .artifact_library
        .latest_signed(&kind, operator_id)
        .await
    {
        Ok(Some(art)) => (StatusCode::OK, Json(art)).into_response(),
        Ok(None) => (
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({"error": "no signed artifact for this kind"})),
        )
            .into_response(),
        Err(e) => library_err(e),
    }

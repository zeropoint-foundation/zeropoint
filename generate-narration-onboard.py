...
NARRATIONS = [
    ...
    (
        ...
        "Here's the first real departure from the old model. "
        "Traditionally, proving your identity means presenting credentials "
        "issued by someone else. A password stored in a corporate database. "
        "A token granted by an OAuth provider. A certificate signed by a "
        "certificate authority. Someone else holds the keys to your "
        "identity. They can revoke it, surveil it, or lose it in a breach. "
        "ZeroPoint inverts this. Your Genesis secret, the root of your "
        "entire trust chain, needs a guardian that answers only to you. "
        "A password can be phished, guessed, or stolen from a database. "
        "A file on disk can be copied by any process running as your user. "
        "But a sovereignty provider? That's yours. A fingerprint, a "
        "hardware wallet, Touch ID, a face scan — something that proves "
        "you are physically present. An agent running on your machine, "
        "no matter how capable, cannot fake that proof. "
        "This is what sovereignty-first security looks like in practice. "
        "Your physical presence is the boundary between authorized and "
        "unauthorized. No institution mediates. No platform can revoke. "
        "A Trezor on your desk or Touch ID on your laptop — the form "
        "factor changes, but the principle doesn't. You prove presence. "
        "ZeroPoint trusts the proof. "
        ...
        "front of it changes. "
        "Choose a hardware device or biometric if your system supports it. "
        "Login password is the solid default. File-based is for servers "
        "and CI."
    ),
    (
        "onboard-genesis.mp3",

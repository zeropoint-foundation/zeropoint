...
5.5. Key Hierarchy and Introduction Protocol. ZeroPoint solves the key distribution problem through zp-keys — a three-level certificate hierarchy that exists below the policy engine.

GenesisKey — a self-signed root of trust, one per deployment. OperatorKey — signed by genesis, one per node operator. AgentKey — signed by operator, one per agent instance.
...
It does not solve key discovery. zp-keys solves key distribution — see section 5.5 — but how peers find each other's network addresses remains outside the protocol scope.

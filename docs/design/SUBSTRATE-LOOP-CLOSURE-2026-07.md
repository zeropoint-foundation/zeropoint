...
- **Build the Cartographer first.** *Deferred.* It is the highest-leverage single piece and it is large. S1–S3 are cheaper, and E2's registry plus S3's enumerated read set are direct inputs to its design — you want to know exactly which receipt shapes officers depend on before designing the projection that replaces those reads. Reopen condition: S3 complete, or v0/m0 evidence available. Reopen watch: `receipt_exists(cognitive:act:recorded)` — **satisfied 2026-07-26**: v0/m0 landed (`cognitive:act:recorded` emits at four cycle exits, registered in `substrate_validate.rs`). This tie-off is **eligible for re-proposal against its original arc** and is held open pending operator decision, not pending evidence. Recorded here because the runtime that would emit `improvement:reopen:eligible` does not exist yet — the eligibility was found by hand on the first run of the check that enforces this field.

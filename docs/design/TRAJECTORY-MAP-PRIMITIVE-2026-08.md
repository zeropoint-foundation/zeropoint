...
**Tier 2 canonical elaboration (SKETCH — 2026-08-02).** Proposes a substrate primitive for coordinating multi-cycle, multi-arc work whose destination may be unknown or emergent. Elaborates `KEEL-2026-07.md` §II (WorkArc / cognitive loop) and §II.18 (chain-watcher and commitments). Composes with `TRAJECTORY-AWARE-CONSTITUTIONAL-ENFORCEMENT-2026-07.md` (Aegis observes map trajectory), `OFFICER-ACTION-SURFACES-2026-07.md` (five-phase ceremony per ticket), `SUBSTRATE-SELF-CONSTRUCTION-2026-07.md` (builder dispatch executes tickets), `EXECUTION-AUTHORITY-MODEL-2026-07.md` (novel-vs-precedent gate for ticket types), `SHADOW-EVALUATION-PRIMITIVE-2026-07.md` (prototype tickets dispatch to shadow-eval), `ARTIFACT-LIBRARY-2026-05.md` (destinations are artifacts under the standard lifecycle), `CHAIN-WATCHER-AND-COMMITMENTS-2026-07.md` (blocking relationships are commitments). Canonical claims live in KEEL; this sketch would need promotion + KEEL amendment to become canon.
...
The trajectory map primitive is **not a peer struct to WorkArc — it is what WorkArc grows into when it takes on tickets**. Every arc is a map with zero tickets. A directive that stays arc-shaped just never opens any tickets and its `tool_history` alone carries the work. A directive that opens tickets promotes the same arc into map mode; the ticket set, blocking relationships, heading, trajectory, and destination hypotheses are fields on the existing `WorkArc`. This is a structural continuity choice, not a modeling one — it preserves the invariant that "a WorkArc is one unit of coherent work the Regent is doing" and lets the map affordances be lit up incrementally rather than requiring a whole parallel machinery for the fog-shaped case.
...
The trajectory map primitive names both classes as first-class substrate work. A map has a **heading** (operator-signed, always present, the directional signal), an evolving set of **tickets** (research, prototype, grilling, task — each a scoped sub-arc dispatchable through existing builder machinery), an emergent **trajectory** (the projection over resolved tickets that shows where the work is heading), and — optionally, and only if it crystallizes — a **destination** (which is an artifact under `ARTIFACT-LIBRARY` lifecycle, not a mutable map field). Nothing about the map violates append-only. The map itself is a query over receipts.
...
- Trajectory is observable at any point — it's the projection over the map's resolved tickets. Aegis watches it (per `TRAJECTORY-AWARE-CONSTITUTIONAL-ENFORCEMENT-2026-07`) and surfaces divergence to the operator.
...
Scoped sub-arcs of work. Each ticket has:
...
- **Rationale**: why this ticket, what fog it's clearing.
- **Blocking set**: receipt ids that must exist in the chain before this ticket becomes takable. Empty set means the ticket is on the frontier. Non-empty means it's in fog awaiting resolution.
- **Expected outcome**: what the ticket's resolution receipt is expected to contain (used by Aegis to detect trajectory divergence when actual outcomes don't match).

Tickets are opened via `arc:ticket:opened` receipts (map-scoped). Resolved via `arc:ticket:resolved` receipts that reference the opening receipt and carry the outcome. Tickets themselves follow the five-phase officer ceremony (`OFFICER-ACTION-SURFACES` §"five-phase action ceremony lifecycle") — a ticket's resolution IS the ceremony completing.

### Trajectory

The projection over resolved tickets. Not a field — a query. Given a map, its trajectory is: the sequence of ticket resolutions in chain order, with each resolution treated as a directional signal (either confirming the heading or bending it). Aegis reads the trajectory continuously and emits `aegis:action:trajectory_alert` if the trajectory diverges beyond a threshold from the heading, or from prior maps in the same heading area.

### Fog

The set of tickets known to be needed but not yet takable (blocking set non-empty). Fog is a first-class state — a ticket in fog is not a defect, it's a named uncertainty the map is tracking. When a fog ticket's blockers all resolve, it promotes to the frontier automatically (via query, not receipt).

### Frontier

The set of tickets currently takable (blocking set empty, not yet resolved). This is the primary operator-and-Regent view of "what can we do next in this map." Empty frontier + non-empty fog means the map is waiting on the resolution of blocking tickets. Empty frontier + empty fog means the map has run out of tickets — either the destination emerged (settle with artifact) or the exploration converged (settle with abandonment + rationale).
...
3. For a map-shaped directive, Regent emits `arc:map:proposed` naming a heading, an optional destination hypothesis, and an initial ticket set. This is an approval-request per the existing approvals machinery.
4. Operator signs `arc:map:opened` (perhaps modifying the initial ticket set or heading). The map is now open.
5. Regent begins working the frontier.

### Working the frontier

Each cycle, Regent picks a takable ticket (heuristic — probably deepest-current-fog-clearer or operator-prioritized-if-any) and dispatches it via the appropriate mechanism per its type. On resolution, the ticket's outcome may:

- Confirm the current trajectory (no map change).
- Open new tickets (fog resolves into concrete work). Each new ticket is an `arc:ticket:opened` receipt referencing the resolving ticket. Novel ticket types (types the map's precedent hasn't seen) escalate to operator per EXECUTION-AUTHORITY-MODEL; precedent-matching types auto-proceed.
...
- Add a ticket (`arc:ticket:opened` operator-signed).
- Reprioritize the frontier (`arc:map:priority_hint`).
- Reject a Regent-proposed ticket (`arc:ticket:rejected`).
- Accept or reject a destination proposal.
- Split a ticket into multiple (`arc:ticket:split` referencing the original).
...
- **`OFFICER-ACTION-SURFACES-2026-07`.** Each ticket is an officer-action-shaped work item. The five-phase ceremony (written → executed → tested → verified → signed) applies per ticket. The map coordinates tickets; ticket ceremony is unchanged.

- **`SUBSTRATE-SELF-CONSTRUCTION-2026-07`.** Builder dispatch is the mechanism for research and task tickets. The Regent charts the map; builders execute individual tickets; results return to the Regent's chain.

- **`SHADOW-EVALUATION-PRIMITIVE-2026-07`.** Prototype tickets dispatch to shadow-evaluation. A prototype ticket's resolution IS a shadow-eval outcome. This is the antidote to waterfall — high-fidelity feedback on candidate directions before committing to them.

- **`EXECUTION-AUTHORITY-MODEL-2026-07`.** The precedent-vs-novel gate applies at map-open (does this directive have precedent for map-shape?) and at ticket-open (does this ticket type have precedent for auto-proceed?). The Regent's Phase 7 states (`regent:proposal:*`) compose naturally with map ceremony.
...
- **`CHAIN-WATCHER-AND-COMMITMENTS-2026-07`.** Blocking relationships between tickets are commitments — a ticket in fog is a `check-at` commitment waiting on the presence of specific resolution receipts. This is not a separate mechanism from commitments; it IS commitments applied at map scope. Worth deciding whether to unify or keep parallel.
...
**Map is an extension of WorkArc, not a peer primitive.** Every arc is a map with zero tickets. Ticket-set growth is what promotes an arc into map mode. Field additions on `WorkArc` (ticket set, blocking relationships, heading marker, destination hypotheses, trajectory projection cache if any) are all optional-null for the arc-shaped case, so existing linear WorkArc behaviour is untouched. See "Framing" and "Heading" sections for the load-bearing consequences.

**Multi-operator maps are supported.** A map opened by one operator can be contributed to or continued by another operator with delegation to the arc. The chain structurally supports this; the ceremony detail (whose signatures matter when — for destination acceptance, ticket rejection, heading amendment) will need spec work but the answer is "multi-operator, not single-operator-locked."

...
**Update 2026-08-17 — decided, in part.** `PERENNIAL-GRANT-2026-08.md` took E13's constructor finding one layer down and asked where the default actually lands. Answer: two production sites in `zp-server` issue wildcard `ToolCall` grants that take it, and both defended the choice in their own doc comments — so this was not an omission anywhere it mattered, it was a position. Ruled 2026-08-17: both sites now attach `LeasePolicy::standard_8h()`, and a `zp revoke --grantee` verb was added because the memo found that revoking an agent had been proportional to how many times it had ever launched.

What that does and does not settle. It closes the instance and leaves the mechanism: **the constructor default is unchanged**, the `validate_issuance` remedy this sheet named is still unimplemented, and a caller who omits an expiry still gets an unbounded grant. E13's "fails at the constructor" therefore remains accurate about the constructor and is no longer accurate about the shipped configuration — a distinction worth keeping, because the two get closed by different work.

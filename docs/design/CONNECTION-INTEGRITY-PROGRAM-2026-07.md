...
- **Extend graphify rather than build `tools/connection-map/`.** *Declined.* graphify already extracts code→code edges and has a query interface. It does not model corpus, chain, config, or artifact edges, and teaching it to would be a larger change than collating seven sources. Reopen condition: P1 complete, at which point the overlap is measurable rather than guessed. Reopen watch: `artifact_exists(tools/connection-map/connections.json)` — **satisfied 2026-07-26**; the branch reopened and now declines on the merits rather than on cost. With P1 built, the overlap is measurable and it is zero: graphify's 358,825 links are *derived*, and a connection is a *declared* dependency, so the two tools answer different questions. Superseded reason recorded rather than replaced.

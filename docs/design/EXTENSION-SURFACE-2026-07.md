...
- Third-party attestations accompanying the artifact, if any — presented as evidence with the attesting party named, never as a substitute for the operator's decision (see below)
...
**Third-party attestation as evidence, never authority (added 2026-08-14; `AI-LANDSCAPE-SIGNAL-2026-07.md` §"Signal 5", E9a).** The distribution model below states that registries are advisory and that the substrate trusts the individual signature and author, not the index. A vendor-signed capability artifact is the first thing that tests what "advisory" means mechanically: NVIDIA-Verified Agent Skills ship a detached signature over the whole artifact directory, verifiable against a vendor root, plus a machine-readable *skill card* naming ownership, licence, dependencies, known limitations, risks and mitigations. Under QUARANTINE-PLANE Step 2 as written such an artifact is simply **unsigned** — signatures are verified against Genesis-derivable signers, the check is binary, and a verifiable signature by a non-Genesis party has no third state. Its evidence is discarded rather than recorded, which is a loss, not a defence.

The shape that keeps the discipline is a third state that carries no authority:

- The substrate verifies the attestation cryptographically and records **what it is** — attesting party, signature verification result, the claims the attestation makes — in a `quarantine:attestation:<surface>:<hash>` receipt carrying an explicit `authority: none` marker.
- The receipt is presented in the review surface above, next to the author key and the operator's own delegation history. It is read by the operator and **never consumed by the gate**. No admission advances because an attestation is present, and none is blocked because one is absent.
- Chain-anchoring it is the point: if an attesting party is later found compromised, everything it vouched for is enumerable. An attestation the substrate merely displayed and discarded leaves nothing to enumerate.

**Constraint that travels with it.** A third-party attestation must not become a precedent source. QUARANTINE-PLANE §Open positions carries precedent-based auto-admission — admitting from signer X after the operator has admitted from X before — and *act on precedent, escalate on novelty* requires that the precedent be built from **operator signatures**. Precedent built from a vendor's signatures is the vendor's trust root wearing the operator's clothes, which is the substitution `draft-reece-wimse-cross-org-delegation` §7 names as the high-value target. Per `SUBSTRATE-FORM` §"Trust-chain reach is stated per layer", this receipt is also the record from which a substrate would derive whether its capability-admission root has diverged from its boot root, and therefore whether Form Disclosure is owed.
...
- **Declared purpose versus requested scope — advisory only, or not at all (2026-08-14).** The capability audit above is deliberately *structural*: WASM imports versus manifest, a check that cannot be argued with because the extension physically cannot import what it did not declare. It catches a declaration that lies about what the code imports; it cannot catch one that is structurally honest and semantically false — a capability whose `justification` string describes one purpose while the requested scope serves another. The substrate already collects the raw material, since every capability carries an operator-readable justification, and **nothing reads them**. NVIDIA's pre-publication scanner checks exactly this mismatch (declared purpose versus requested access versus bundled behaviour), which makes the gap externally instantiated rather than theoretical. The open question is not whether the check is possible but whether it can be built without damage: a semantic comparison is policy-based and fallible, and wiring it into admission converts a structural gate into a judged one. Recommended position: **advisory input to the operator ceremony, never a gate** — it produces a finding the operator reads and never blocks an admission. If that constraint cannot be held, do not build it. Costed against QUARANTINE-PLANE §Open positions "verification cost bounds": this is an inference call per capability per admission, in the expensive class.
- **Instruction-shaped artifacts that carry authority (2026-08-14).** The eight trait families above are all code, and the structural audit primitive — declared imports versus actual imports — has no analogue for an instruction bundle, where there are no imports to diff. The industry's capability unit at this layer is now exactly that: portable instruction sets, signed, catalogued and shipped with a machine-readable trust record. QUARANTINE-PLANE's canonical-spec surface does have a home for prompts and policy modules arriving as artifacts, but its verification schema is schema conformance plus corpus cross-reference, which says nothing about what an authority-carrying instruction asks the cognition layer to do. Per `HARNESS-SEAM-2026-08` §2.2 skill definitions are **outer content**: the substrate has an admission ceremony for inner-shaped artifacts and crossing-observability for outer configuration, and no ceremony for outer instruction artifacts carrying authority. `FOOTPRINT-AUDIT-2026-04` logged the same hole from the other side under LLM03 / ASI04 ("no signature verification on skill packages, no SBOM"). This wants a design pass, not a note. **Naming note:** `zp-skills` is scheduled for deletion in the W5 work (`LEGACY-ACCOUNTING-2026-08` §3.3.1 — a substring matcher with one consumer, agent-framework residue). That deletion is correct and unaffected; it happens to free the name as the industry fixes its meaning. Any later admitted-instruction-artifact primitive is not a reversal of it and should not inherit the name.

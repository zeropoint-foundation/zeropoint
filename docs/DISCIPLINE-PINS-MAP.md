...
- **Widest allowlists** (4 with ≥5 exemptions): `no_raw_peer_url_outside_zp_net` (17), `no_raw_variable_spawn_outside_zp_host` (11), `singular_sovereign_root` (7), `no_raw_tcp_bind_outside_zp_net` (6) — each exemption is a place the rule does not hold. Worth re-reading when the count grows, since exemptions are added one at a time and never reviewed together.
...
- **Described by `docs/DISCIPLINE-PINS.md`** (2 of 28): `no_raw_peer_url_outside_zp_net`, `singular_sovereign_root`

...
DECIDED-006 DATE: 2026-08-16.
DECIDED-006 DECISION: the ZeroPoint Open Foundation workspace is set aside. Foundation workspace users are parked. It is to be re-established on Cloudflare and Hetzner once the substrate reaches deployable maturity. Operator decision, not a failure of anything.
DECIDED-006 SCOPE: this is a deployment posture, not a code change. The onboarding wizard remains implemented in the foundation worker (`zeropointfoundation.org/onboard/`, worker plus frontend). Nothing is deleted. What changes is that nothing is currently serving `onboard:start` / `onboard:complete`, so `scripts/verify-onboarding-receipts.sh` has no live target until redeployment.
DECIDED-006 WHY: the substrate is not yet deployable, so a running foundation workspace has nothing mature to front. Standing up user-facing infrastructure ahead of the thing it fronts creates an obligation to keep it alive through every substrate change.
DECIDED-006 CONNECTS: `crates/zp-server/src/substrate_validate.rs`, the `NOT RESERVED — onboard:` comment block. That comment was amended the same day to name three states rather than two: implemented, invisible to `connection_map` (which scans `crates/**/*.rs` only and cannot see a non-Rust implementation), and NOT DEPLOYED. Before the amendment it read as though the wizard were running.
DECIDED-006 CONNECTS: the six `onboard:` edges that `connection_map` reports as `corpus_to_chain` / aspirational. They are deliberately left as defects. Reserving them would record built work as deferred roadmap; treating them as live would claim a deployment that does not exist. Unclassified is the honest state for code that exists, is not visible to the scanner, and is not deployed.
DECIDED-006 REJECTED: recording this as a pre-declaration in `BUFFER-OBSERVATION-2026-08` §5. That section requires a computable `reserve_measure` and states outright that an entry without one does not belong. A deployment deferral is not a depleting reserve — nothing drains while the workspace is down — so it fails §1's own buffer test. Proposed in conversation and withdrawn on reading §5's admission rule.
DECIDED-006 OPEN: "deployable maturity" is the precondition and it is not defined by any test. There is no receipt, gate, or number that fires when the substrate becomes deployable, so the condition for reversing this decision is currently a judgment. `SUBSTRATE-READINESS-CONTRACT-2026-07` is the nearest existing frame and its receipts are reserved-not-built (`substrate:readiness:`). Naming a concrete precondition is outstanding work.
DECIDED-006 NOT-CLAIMED: this does not say the foundation surface was wrong to build, and does not commit to rebuilding it in the same shape. It records that it is down by choice, and that three independent things would each resolve the `onboard:` reporting gap — extending the scanner past `crates/`, the foundation-side receipt signing that `verify-onboarding-receipts.sh` names as open work, or redeployment. Only the third is gated on this decision.

DECIDED-007 DATE: 2026-08-16.
DECIDED-007 SUBJECT: first reading for `HOST-ATTRIBUTION-CLOSURE-2026-08` §8, run on APOLLO-4 via `tools/attribution-probe/first_reading.py`. Recorded because TRIAGE-FOR-COHERENCE holds that a new measure's first reading is evidence about the measure, not about the system — and a delta cannot be read against a reading nobody wrote down.
DECIDED-007 READING: window 1637s, 790 samples at 2.0s, 1718 distinct (pid,start), 938670 process-seconds. By location: path-unresolved 912 distinct / 12.0% proc-sec; os-baseline 597 / 65.2%; app-bundle 198 / 21.4%; user-app 7 / 0.7%; unaccounted 2 / 0.3%; homebrew 2 / 0.3%. Seen in exactly one sample: 810 of 1718 (47.1%).
DECIDED-007 DEFECT-1: the probe called `ps` twice and joined on pid. A process exiting between the calls kept its pid/ppid/lstart and lost its path, landing in a bucket named `kernel-or-pathless` — 912 records, 53.1%, the largest single category in the reading. That population tracks the 47.1% single-sample population closely, and zero single-sample records were classified `unaccounted`. The instrument was systematically blind to short-lived processes, which are the population the measure exists to catch, and filed them under a name that reads as benign. The join was never necessary: `comm` is the last `ps` field, so one call suffices.
DECIDED-007 DEFECT-2: the dominance test printed "DOMINATED — this is not a general attribution measure and should be renamed" from n=2, where the larger of two necessarily holds 50%. That verdict was arithmetic, not evidence. Gated at n=20 in v2.
DECIDED-007 DEFECT-3: attribution was by path prefix, so `/Users/` vouched for anything in the operator's home directory and `/var/` for anything staged. HOST-ATTRIBUTION-CLOSURE §4.2 requires attribution to name executable identity, authorizing delegation, and expected behaviour class; a prefix supplies none of the three. The ruleset was non-compliant with the specification it was written to test. This is §6's liberal-attribution cheat, authored into the default ruleset one turn after the warning was written.
DECIDED-007 SURVIVED: two findings survived all three defects. `/Library/PrivilegedHelperTools/com.docker.vmnetd` — a privileged root helper for Docker networking, unaccounted. And `/usr/local/bin/zp` — see QUESTION-005. Both parented by `/sbin/launchd`. That anything at all escaped a ruleset this permissive is the reason the reading is worth keeping.
DECIDED-007 NOT-CONCLUDED: this reading does NOT establish that ungoverned host accretion is or is not occurring on APOLLO-4. With a ruleset that attributes by location, a near-zero unaccounted count is a statement about the rules. The §8 pre-commitment — that a near-zero result means the measure is premature rather than that the rules should be loosened — is not discharged by this run, because the correction being made runs the other way, toward stricter attribution. Whether that asymmetry is principled or motivated is recorded here rather than settled: the defence is that §4.2 already required provenance and the ruleset implemented location, so v2 implements the spec rather than chasing a result.
DECIDED-007 SUPERSEDED-BY: `tools/attribution-probe/second_reading.py`, which fixes all three defects, adds `codesign`-based attribution as the primary provenance key, and retains the path ruleset alongside it so the difference between attribution-by-location and attribution-by-signature is reportable as a number rather than asserted.
DECIDED-007 NOT-CLAIMED: neither probe is substrate code. Both are experiment artifacts, emit no receipts, and should be deleted or deliberately promoted once the ruleset settles.

QUESTION-005 DATE: 2026-08-16. Owner: none.
QUESTION-005 FINDING: `/usr/local/bin/zp` was reported `unaccounted` by the first attribution reading. The substrate cannot account for itself under its own proposed attribution scheme.
QUESTION-005 MECHANISM: the path ruleset covers `/opt/homebrew/`, `/usr/local/Cellar/` and `/usr/local/opt/` but not `/usr/local/bin/`, which is where `zp` is installed. So this is partly a gap in a ruleset already known to be defective (DECIDED-007 DEFECT-3), and reading it as profound would be over-reading a missing prefix.
QUESTION-005 WHY-IT-STILL-MATTERS: the interesting question is not which prefix was missing. It is what the substrate's *own* attribution record should be. `zp` is the one process on the host whose provenance the substrate could state authoritatively — its Genesis lineage, its build hash, its signing key — and the proposed scheme has no category for "this is me." Self-attribution is a distinct state from `governed` and from `attributed`, and neither HOST-ATTRIBUTION-CLOSURE §4.1 nor either probe has one.
QUESTION-005 OPEN: does the closed set need a fourth state for substrate self-attribution, or does `zp` attributing itself create a circularity that should be resolved by external attestation instead? Unowned. Composes with SUBSTRATE-BOOT-INVARIANT-CEREMONY (build-hash provenance already exists at boot) and HOST-BROKER (`no_raw_spawn_outside_zp_host` makes zp the parent of governed processes, which makes its own attribution load-bearing for parent-attribution rules).

DECIDED-008 DATE: 2026-08-16.
DECIDED-008 SUBJECT: second reading for `HOST-ATTRIBUTION-CLOSURE-2026-08` §8, run on APOLLO-4 via `tools/attribution-probe/second_reading.py`. Window 2159s, 1052 samples at 2.0s, 2802 distinct (pid,start), 510 distinct executables, 1224978 process-seconds. Stopped on saturation (120 samples with no previously-unseen executable), not on a timer.
DECIDED-008 DEFECT-4 — THE INSTRUMENT WAS COUNTING ITSELF: 1052 of the 1281 `path-unresolved` records were `ps` — the probe's own sampling calls, one per sample, exactly matching the 1052 samples taken. 82% of the report's largest category was the instrument observing itself. Self-excluded, unresolved falls from 45.7% to 13.1% by count and the single-sample rate falls from 48.9% to 18.2%. By process-seconds the correction is negligible (12.5% -> 12.4%), because `ps` is short-lived. Fixed by excluding on parentage (`ppid == os.getpid()`) rather than by command name.
DECIDED-008 MISDIAGNOSED-TWICE: this same bucket was called `kernel-or-pathless` in v1 and read as benign, then diagnosed in DECIDED-007 DEFECT-1 as a two-call join race and "fixed" accordingly. Both diagnoses were wrong and the second was asserted with confidence. The v2 report's own §0 criterion — "if path-unresolved is still near the single-sample rate, the race was not the cause and the bucket is real" — returned the correct first half and the wrong second half: the bucket was neither the race nor real. A category that has been renamed twice and re-explained twice without anyone displaying its contents is the finding. The fix that mattered was one line of analysis against the saved JSON, not either code change.
DECIDED-008 COUNT-AND-TIME-DISAGREE: by count the unresolved bucket is `ps` (82%). By process-seconds it is `login` and `-zsh` at 37.1% each — 27 login shells, pathless because macOS renders a login shell as `-zsh` — together 74% of unresolved time. The two orderings share no top entry. This is why HOST-ATTRIBUTION-CLOSURE §4.3 specifies both `unaccounted_process_seconds` and `distinct_unaccounted_executables`; the reading demonstrates the specification was right for a reason the specification did not anticipate.
DECIDED-008 RULESET-GOT-LOOSER: v2 added `library:`, `local-bin:` and `priv-helper:` path categories. Those swallowed both of v1's only findings — `com.docker.vmnetd` became `priv-helper`, `/usr/local/bin/zp` became `local-bin` — and drove `unaccounted` to exactly zero. Location-based attribution is now saturated on this host: every path matches some prefix. That is the tautology DECIDED-007 DEFECT-3 predicted, arrived at by making the ruleset MORE permissive in the same edit that complained it was too permissive.
DECIDED-008 SIGNATURE-IS-THE-MEASURE: by code signature, apple-platform 63.9% of process-seconds, Developer-ID teams ~22% across 8 distinct team identifiers, adhoc 1.4%, unsigned-or-unknown 12.5%. This is the first attribution figure on this host that reflects provenance rather than filesystem location, and it is the only one of the two that should be carried forward.
DECIDED-008 FINDING — GOVERNANCE BINARIES ARE UNATTRIBUTABLE: 56 distinct executables were attributed by location but are ad-hoc signed, which vouches for nothing. They are the Rust toolchain (`rustc`, `cargo`, `clippy-driver`, `rustfmt`), anaconda and uv Pythons, and ZeroPoint's own build artifacts — including the discipline-pin test binaries `no_raw_home_lookup`, `no_raw_tcp_bind_outside_zp_net`, `no_raw_storage_calls_outside_zp_content` and `adapters_must_be_documented`. The binaries that enforce the substrate's governance pins are themselves unattributable under the substrate's own proposed scheme. This generalizes QUESTION-005 from `zp` to the whole locally-built toolchain.
DECIDED-008 CONSEQUENCE-FOR-OBSERVE-PROCESS: `observe:process:` is reserved-not-built. When it is built it MUST exclude the observation plane's own enumeration, by parentage, or its findings will be dominated by its own footprint exactly as this probe was. That constraint was discovered empirically before the family exists, which is what a first reading is for. Record it against the reservation so it graduates with the constraint attached.
DECIDED-008 NOT-CLAIMED: this still does not establish whether ungoverned host accretion occurs on APOLLO-4. Under a saturated location ruleset `unaccounted` is zero by construction, and the signature-based figures have not been run as a closure measure. Both readings remain evidence about the measure.
